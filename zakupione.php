<?php
    ...
    if($polaczenie->connect_errno!=0){
		...
		if( $var1 == 3)
		{
            $sql = " SELECT uzytkownicy.imie, uzytkownicy.nazwisko, uzytkownicy.email, uzytkownicy.numer, uzytkownicy.user, pakiet_ubezpieczenia.nazwa_ubez, typ_ubezpieczenia.nazwa_typu, typ_ubezpieczenia.cena FROM uzytkownicy INNER JOIN pakiet_ubezpieczenia ON uzytkownicy.ubezID=pakiet_ubezpieczenia.id INNER JOIN typ_ubezpieczenia ON pakiet_ubezpieczenia.typID=typ_ubezpieczenia.id WHERE uzytkownicy.id >= 1 ";
            $result = $polaczenie->query($sql);
            $polaczenie->close();
        }else{
            $sql = " SELECT uzytkownicy.imie, uzytkownicy.nazwisko, uzytkownicy.email, uzytkownicy.numer, uzytkownicy.user, pakiet_ubezpieczenia.nazwa_ubez, typ_ubezpieczenia.nazwa_typu, typ_ubezpieczenia.cena FROM uzytkownicy INNER JOIN pakiet_ubezpieczenia ON uzytkownicy.ubezID=pakiet_ubezpieczenia.id INNER JOIN typ_ubezpieczenia ON pakiet_ubezpieczenia.typID=typ_ubezpieczenia.id WHERE uzytkownicy.id = $var1 ";
            $result = $polaczenie->query($sql);
            $polaczenie->close();
        }
    }
    ...
    <section>
        ...
        <table>
            <tr>
                ...
                <th>Login</th>
                <th>Nazwa typu ubezpieczenia</th>
                <th>Nazwa pakietu</th>
                <th>Cena pakietu</th>
            </tr>
